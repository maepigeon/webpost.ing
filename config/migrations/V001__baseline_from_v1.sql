-- =============================================================
-- V001 — Baseline migration from the original v1 schema
--
-- Starting state: only users, posts, users_posts_junctions exist
-- with narrow column types (password VARCHAR(32), description TEXT).
-- This migration brings the database to the state before V002.
...
-- -------------------------------------------------------------
-- users — widen password for BCrypt and add all new columns
-- -------------------------------------------------------------
...
-- -------------------------------------------------------------
-- posts — widen description to TEXT, add new columns
-- -------------------------------------------------------------
...
-- -------------------------------------------------------------
-- role_limits table + seed data (roles: user, trusted, restricted, admin)
-- -------------------------------------------------------------
...
-- -------------------------------------------------------------
-- post_reactions — create or upgrade PK to allow multiple per user
-- -------------------------------------------------------------
DO $$
BEGIN
    ...
    ELSE
        ...
        -- Suppress duplicate-key error if triple PK already exists
        BEGIN
...
-- -------------------------------------------------------------
-- comment_reactions — create or upgrade PK to allow multiple per user
-- -------------------------------------------------------------
DO $$
BEGIN
